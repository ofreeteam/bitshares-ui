...
In order to increase your exposure to NBS and offer liquidity to BitAssets, such
...
The NewBitShares network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 NBS as it is traded on external exchanges.
...
The NewBitShares network is capable of margin calling those positions that do not
...
offer to buy the collateral is less than the call price (x/NBS).
...
market rates up to the SQP and close the position. The remaining NBS of the
...
increase, an additional amount of NBS is locked as collateral, while reducing
...
particular bitAsset to hand it over to the NewBitShares network. After that, the
